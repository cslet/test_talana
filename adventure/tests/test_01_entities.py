...
class TestVehicle:
    ...
    def get_distribution(passengers):
        vl_pasajeros = models.Vehicle.objects.get(name='car')
        arrayBool = list()
        for i in range(1, (vl_pasajeros.passengers + 1)):
            # print(i)
            if ( i % 2 != 0 and i == vl_pasajeros.passengers):
                arrayBool.append([True, False])
                break
            elif(i % 2) :
                arrayBool.append([True, True])
        return arrayBool
        # assert TestVehicle.get_distribution() == arrayBool
    ...
    def test_vehicle_distribution(self, car, van):
        ...
        assert vehicle.get_distribution()
...
class TestJourney:
    ...
    # def is_finished():
    def is_finished(vehicle):
        vehicle_name = models.Vehicle.objects.get(name=vehicle)
        busca_vehiculo = models.Journey(end=date.today(), vehicle=vehicle_name)
        finalizado = busca_vehiculo.vehicle        
        return finalizado
